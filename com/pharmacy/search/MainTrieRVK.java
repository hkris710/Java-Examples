...
public class MainTrieRVK {
	...
	static class TrieNode{
		int index;
		HashMap<String, TrieNode> subTries = new HashMap<String, TrieNode>();
		...
		public void addNode(String name, int index) {
			...
			TrieNode node;
			if (subTries.containsKey(first)) {
				node = subTries.get(first);
			}
			else {
				node = new TrieNode(index);
				subTries.put(first, node);
			}
			if (name.length() > 1) {
				node.addNode(name.substring(1, name.length()-1), index);
			}
		}
		...
		public int findIndex(String name) {
			...
			for (int ix=0; ix < letters.length && node != null; ix++) {
				node = node.subTries.get(Character.toString(letters[ix]));
			}
